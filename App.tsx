...
const App: React.FC = () => {
  ...
  useEffect(() => {
      ...
      if (loggedInUserStr) {
        ...
        // ADMIN 2.0: Default to STUDENT_DASHBOARD for everyone
  ...
  const handleLogin = (user: User) => {
    ...
    localStorage.setItem('nst_has_seen_welcome', 'true');
    // ADMIN 2.0: Redirect Admin to STUDENT_DASHBOARD by default
  ...
  const handleReturnToAdmin = () => {
      ...
      setState(prev => ({ ...prev, user: prev.originalAdmin, originalAdmin: null, view: 'STUDENT_DASHBOARD', selectedBoard: null, selectedClass: null }));
  };
  ...
  const goHome = () => {
     if (state.user?.role === 'STUDENT' || state.originalAdmin) {
         ...
         setState(prev => ({...prev, view: 'STUDENT_DASHBOARD'})); // Force Admin to Unified Dashboard too
     }
  };
  ...
  const goBack = () => {
    ...
    setState(prev => {
      ...
      if (prev.view === 'CHAPTERS') {
          ...
          if (prev.user?.role === 'STUDENT' || prev.originalAdmin || prev.user?.role === 'ADMIN') {
      ...
      if (prev.view === 'BOARDS') {
          const nextView = 'STUDENT_DASHBOARD'; // Always unify
          return { ...prev, view: nextView as any, selectedBoard: null };
      }
      
      return { ...prev, view: 'STUDENT_DASHBOARD' as any }; // Default fallback
    });
  };
    ...
    <div className="min-h-screen flex flex-col bg-white font-sans relative">
      ...
      <main className={`flex-1 w-full max-w-6xl mx-auto ${isFullScreen ? 'p-0' : 'p-4 mb-8'}`}>
        {!state.user ? (
            ...
            <>
                {/* 
                   ADMIN 2.0: We REMOVE separate AdminDashboard route here. 
                   Admin renders StudentDashboard with extra props.
                   AdminDashboard is now rendered as an overlay inside StudentDashboard.
                */}
                ...
                {activeWeeklyTest ? (
                    ...
                    (state.view === 'STUDENT_DASHBOARD' || state.view === 'ADMIN_DASHBOARD') && (
                        <StudentDashboard 
                            user={state.user} 
                ...
                {state.view === 'SUBJECTS' && state.selectedClass && <SubjectSelection classLevel={state.selectedClass} stream={state.selectedStream} board={state.selectedBoard} user={state.user} onSelect={handleSubjectSelect} onBack={goBack} />}
                {state.view === 'CHAPTERS' && state.selectedSubject && <ChapterSelection chapters={state.chapters} subject={state.selectedSubject} classLevel={state.selectedClass!} stream={state.selectedStream} board={state.selectedBoard} loading={state.loading && state.view === 'CHAPTERS'} user={state.user} onSelect={onChapterClick} onBack={goBack}/>}
                {state.view === 'LESSON' && state.lessonContent && <LessonView content={state.lessonContent} subject={state.selectedSubject!} classLevel={state.selectedClass!} chapter={state.selectedChapter!} loading={false} onBack={goBack} user={state.user} />}
            </>
        )}
      </main>
      
      {/* PERSISTENT FOOTER - Hide in Student Dashboard as it has its own Bottom Nav */}
      {!isFullScreen && state.view !== 'STUDENT_DASHBOARD' && state.view !== 'ADMIN_DASHBOARD' && (
