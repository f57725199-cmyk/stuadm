...
import { saveUserToLive, auth, getUserByEmail, rtdb, isUserDatabaseEmpty } from '../firebase';
...
export const Auth: React.FC<Props> = ({ onLogin, logActivity }) => {
  ...
  useEffect(() => {
      ...
      if (s) {
          ...
          // ADMIN 2.0 BOOTSTRAP: Default settings if DB is empty
          setSettings({
              adminEmail: 'admin@iic.com', // HARDCODED FALLBACK FOR SETUP
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (view === 'LOGIN') {
      ...
      try {
          ...
          // CRITICAL FIX: Prevent Admin Demotion
          // If the logging-in email matches the configured Admin Email, FORCE Admin Role.
          // This prevents accidental demotion if the user was fetched as STUDENT due to sync errors.
          // ADMIN 2.0: Added fallback for bootstrap 'admin@iic.com'
          const isAdminEmail = (settings?.adminEmail && loginEmail.toLowerCase() === settings.adminEmail.toLowerCase()) || 
                               (loginEmail.toLowerCase() === 'admin@iic.com');
          
          if (isAdminEmail) {
              if (appUser.role !== 'ADMIN') {
                  console.log("⚠️ Detected Admin Email with wrong role. Auto-Correcting to ADMIN.");
                  appUser.role = 'ADMIN';
                  appUser.credits = 99999;
                  appUser.subscriptionTier = 'LIFETIME';
                  // Immediately fix the cloud record
                  await saveUserToLive(appUser);
              }
          }
      ...
      try {
          ...
          const isFirstUser = await isUserDatabaseEmpty();
          ...
          const newUser: User = {
            ...
            role: assignedRole,
            createdAt: new Date().toISOString(),
            credits: isFirstUser ? 99999 : (settings?.signupBonus || 2), // Unlimited credits for Admin
            streak: 0,
            lastLoginDate: new Date().toISOString(),
            redeemedCodes: [],
