...
import { User, Subject, StudentTab, SystemSettings, CreditPackage, WeeklyTest, Chapter, ViewState } from '../types';
...
import { AdminDashboard } from './AdminDashboard'; // ADMIN 2.0 INTEGRATION
...
interface Props {
  ...
  onNavigate?: (view: ViewState) => void; // Added for Admin Switch
  // ADMIN 2.0 PROPS
...
export const StudentDashboard: React.FC<Props> = ({ 
    ...
    onUpdateSettings, logActivity, onImpersonate 
}) => {
  ...
  const [showAdminOnline, setShowAdminOnline] = useState(false); // Admin 2.0 Feature
  const [showAdminPanel, setShowAdminPanel] = useState(false); // Admin 2.0 Overlay
  ...
  const handleSwitchToAdmin = () => {
    // Admin 2.0: Open Overlay instead of navigating away
      ...
      if (activeTab === 'COURSES') {
          return (
              <div className="space-y-6 pb-24">
                  ...
                  {/* UNIFIED SUBJECT SELECTION (ADMIN 2.0 READY) */}
                  <SubjectSelection 
                      classLevel={user.classLevel || '10'} 
                      stream={user.stream || null}
                      board={user.board || 'CBSE'}
                      ...
                      hideBack={true}
                      onSelect={(subject) => {
                          // Open Unified Chapter Selection
                          setSelectedSubject(subject);
                          setLoadingChapters(true);
                          setContentViewStep('CHAPTERS');
                          // Fetch
                          fetchChapters(user.board || 'CBSE', user.classLevel || '10', user.stream || 'Science', subject, 'English')
                              .then(setChapters)
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20">
        {/* ADMIN 2.0 FLOATING CONTROLS */}
        {user.role === 'ADMIN' && (
             <div className="fixed bottom-24 right-4 z-50 flex flex-col gap-3">
                 {/* 1. User Monitor */}
                 <button 
                    onClick={() => setShowAdminOnline(!showAdminOnline)}
                    className="bg-slate-800 text-white p-3 rounded-full shadow-xl border border-slate-600 hover:bg-slate-700 flex items-center gap-2"
                 >
                     <UserIcon size={20} className="text-green-400" />
                     <span className="font-bold text-xs">Users</span>
                 </button>

                 {/* 2. Global Settings (Switch to old dashboard for config) */}
                 <button 
        ...
        {showAdminPanel && user.role === 'ADMIN' && (
            <div className="fixed inset-0 z-[100] bg-white overflow-y-auto animate-in slide-in-from-bottom duration-300">
                <AdminDashboard 
                    ...
                    logActivity={logActivity || console.log}
        ...
        {showAdminOnline && user.role === 'ADMIN' && (
            <div className="fixed bottom-40 right-4 z-50 w-80 animate-in slide-in-from-right fade-in">
                <div className="relative">
                    <button onClick={() => setShowAdminOnline(false)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 shadow-md z-10"><X size={16}/></button>
                    <AdminOnlineUsers />
                </div>
            </div>
        )}
